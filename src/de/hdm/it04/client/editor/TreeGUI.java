...
public class TreeGUI {
	 ...
	 * einem Tree zusammen f�gt
	 * 
	 * @param baugruppe
	 *            : Baugruppe von der wir jeweils die unteren Baugruppen und
	 *            Bauteile dem Baum hinzuf�gen wollen
	 * @param anzahl
	 *            : Anzahl in welcher Menge ein Bauteil oder eine Baugruppe
	 *            ben�tigt wird
	 * @return TreeItem, welches immer dem vorherigen TreeItem hinzugef�gt �ber
	 *         return
	 */
	public TreeItem treerek(Baugruppe baugruppe, int anzahl) {
		...
		if (stueckliste.isEmpty()) {
			...
			for (int i = 0; i < stueckliste.size(); i++) {
				for (int j = 0; j < allBt.size(); j++) {
					if (stueckliste.elementAt(i).getId() == allBt.elementAt(j)
							.getId()) {
						TreeItem root = new TreeItem();
						root.setText(allBt.elementAt(j).getName()
								+ "["
								+ Integer.toString(stueckliste.elementAt(i)
										.getAnzahl()) + "]");
		...
		for (int i = 0; i < stueckliste.size(); i++) {
			for (int j = 0; j < allBg.size(); j++) {
				if (stueckliste.elementAt(i).getId() == allBg.elementAt(j)
						.getId()) {
					// TreeItem root = new TreeItem();
			...
			public void onSelection(SelectionEvent<TreeItem> event) {
				...
				if (result instanceof Baugruppe) {
					...
					ContentContainer.getInstance().setContent(
							new BaugruppeGUI().showAllBaugruppen(bguebergabe));
					...
					btuebergabe.add(bt);
					ContentContainer.getInstance().setContent(
							new BauteilGUI().showAllBauteile(btuebergabe));
