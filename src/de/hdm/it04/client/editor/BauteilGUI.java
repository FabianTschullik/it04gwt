...
/**
 * Die Klasse BauteilGUI ermoeglicht es dem User, Bauteil Objekte zu erstellen
 * und zu verwalten. Hier wird das User Interface dafuer definiert.
 * 
...
public class BauteilGUI {
	 ...
	 * hinzugefuegt und geaendert werden k�nnen
	 */
	...
	/**
	 * Methode um Bauteil ueber Name oder Id zusuchen
	 * 
	 * @return vPanel
	 */
				...
				if (btSuche.matches("[0-9]+")) {
					...
					sms.getBauteil(id, new AsyncCallback<Vector<Bauteil>>() {
						...
						public void onFailure(Throwable arg0) {
							new AlertGUI().load("Bauteil wurde nicht gefunden",
									"red");
						}
	...
	/**
	 * Formular, welches ermoeglicht ein Name und Beschreibung von einem Bauteil
	 * zu aendern bzw. hinzuzufuegen
	 * 
	 * @param bauteil
	 * @return
	 */
			...
			public void onClick(ClickEvent event) {
				...
				sms.updateBauteil(bt, new AsyncCallback<Vector<Bauteil>>() {
					...
					public void onFailure(Throwable caught) {
						new AlertGUI().load(
								"Bauteil konnte nicht gespeichert werden",
								"red");
					...
					public void onSuccess(Vector<Bauteil> result) {
						new AlertGUI().load(
								"Bauteil wurde erfolgreich gespeichert",
								"green");
						ContentContainer.getInstance().setContent(
								new BauteilGUI().showAllBauteile(result));
		...
		btnAbbrechen.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				sms.deleteBauteil(bt.getId(), new AsyncCallback() {
					...
					public void onFailure(Throwable caught) {
						new AlertGUI().load(
								"Bauteil konnte nicht geloescht werden", "red");
	...
	public Widget showAllBauteile(Vector<Bauteil> bauteile) {
		...
		for (int j = 0; j < bauteile.size(); j++) {
			...
			btnLoeschen.addClickHandler(new ClickHandler() {
				...
				public void onClick(ClickEvent event) {
					...
					sms.deleteBauteil(id, new AsyncCallback() {
						...
						public void onFailure(Throwable caught) {
							new AlertGUI().load(
									"Bauteil konnte nicht geloescht werden",
									"red");
						...
						public void onSuccess(Object result) {
							new AlertGUI().load(
									"Bauteil wurde erfolgreich geloescht",
									"green");
			...
			bauteileTable.setText(j + 1, 2, bauteile.elementAt(j)
					.getBeschreibung());
			bauteileTable.setText(j + 1, 3, bauteile.elementAt(j)
					.getMaterialBezeichnung());
			bauteileTable.setText(j + 1, 4, s1);
			bauteileTable.setText(j + 1, 5, s2);
			bauteileTable.setText(j + 1, 6, bauteile.elementAt(j)
