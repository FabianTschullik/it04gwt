...
public class StrukturstuecklisteGUI {
	...
	public Widget showBaugruppeStrukturstueckliste(Vector<Baugruppe> baugruppen) {
		...
		for (int j = 0; j < baugruppen.size(); j++) {
			...
			btnStrukturstuecklisteErstellen
					.addClickHandler(new btnStrukturstuecklisteErstellenClickHandler());
	...
	public class btnStrukturstuecklisteErstellenClickHandler implements
			ClickHandler {

		@Override
		public void onClick(ClickEvent event) {
			// Mit Klick auf diesen Button soll die Strukturstueckliste zur
			// ausgewählten Baugruppe erzeugt und ausgegeben werden.
			// Bereich clearen, Methode aufrufen, um Struktur in HTML zu pressen
			// und anzuzeigen.
			// vPanel.clear();
